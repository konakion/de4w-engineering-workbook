...
class LiveBuffer:
    ...
    The buffer stores the most recent x, y and z values. Once it contains
    ``window_size`` samples, it can return a pandas DataFrame with x, y, z and
    magnitude columns for feature extraction.
    """
    ...
    def __init__(self, window_size: int = WINDOW_SIZE) -> None:
        ...
        self.x_buffer: deque[float] = deque(maxlen=window_size)
        self.y_buffer: deque[float] = deque(maxlen=window_size)
        self.z_buffer: deque[float] = deque(maxlen=window_size)
